...
//			+ "insert into user_topics (user_id,topics )values "; // (default,name,password,phone_number,email)
	...
	/** Get Posts**/
	public static final String getPosts_query = "select posts.post_id,posts.created_timestamp,posts.image,posts.body,posts.title,posts.author_name from posts,post_topics where posts.post_id = post_topics.post_id and posts.created_timestamp < (?) and post_topics.topic_name in (select topic_name from user_topics where user_id = (?) ) order by posts.created_timestamp limit (?) "; //Expand 
